...
class BankingServer:
    ...
    def build_gui(self):
        ...
        tk.Label(title, text="Banking Server", font=("Arial", 18, "bold"),
        ...
        f.pack(pady=15)
        self.start_btn = tk.Button(f, text="Start Server", command=self.start_server,
                                  bg="#27ae60", fg="white", width=15)
        self.start_btn.pack(side="left", padx=5)
        self.stop_btn = tk.Button(f, text="Stop Server", command=self.stop_server,
        ...
        admin.pack(pady=10)
        self.admin_btn = tk.Button(admin, text="Admin View", command=self.show_admin_login,
                                  bg="#e67e22", fg="white", width=15, state="disabled")
        self.admin_btn.pack()
        ...
        self.log_txt = tk.Text(c, height=15, font=("Consolas", 9), bg="#34495e", fg="#ecf0f1", wrap="word")
    ...
    def get_next_account_no(self, conn):
        cursor = conn.cursor()
        cursor.execute("SELECT account_no FROM accounts ORDER BY account_no DESC LIMIT 1")
    ...
    def show_admin_login(self):
        ...
        win.grab_set()
        tk.Label(win, text="Admin Password", font=("Arial", 12), bg="#ecf0f1").pack(pady=20)
        pw_entry = tk.Entry(win, show="*", font=("Arial", 12))
        pw_entry.pack(pady=10)
        tk.Button(win, text="Login", command=lambda: self.verify_admin_login(win, pw_entry.get()),
                  bg="#27ae60", fg="white", width=10).pack(pady=5)
        tk.Button(win, text="Cancel", command=win.destroy, bg="#95a5a6", fg="white", width=10).pack(pady=5)
    ...
    def show_admin_view(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT account_no, name, balance, netbanking, mobile_number FROM accounts")
            for row in cursor.fetchall():
                tree.insert("", "end", values=(row[0], row[1], f"₹{row[2]:.2f}", "Yes" if row[3] else "No", row[4] or "N/A"))
        except Exception as e:
            self.log(f"Admin view error: {e}")
        finally:
            conn.close()

        tk.Button(win, text="Reset Database", command=self.reset_database, bg="#e74c3c", fg="white", width=15).pack(pady=10)
        tk.Button(win, text="Close", command=win.destroy, bg="#95a5a6", fg="white", width=15).pack(pady=10)

    # --- SEND SMS USING FAST2SMS (Bulk V2 API) ---
    def send_sms_via_fast2sms(self, otp, mobile_number):
        message = f"Hi. Your OTP is {otp}. Valid for 5 minutes. - YourBank"
        url = "https://www.fast2sms.com/dev/bulkV2"
        
        # Ensure mobile_number is clean 10-digit string
        ...
        try:
            ...
            self.log("SMS request timed out")
            return False
        except requests.exceptions.RequestException as e:
            self.log(f"SMS network error: {e}")
            return False
        except Exception as e:
            self.log(f"SMS unexpected error: {e}")
            return False
        ...
        try:
            while True:
                ...
                if action == "register_account":
                    ...
                    self.log(f"Register attempt: name={name}, deposit={deposit}, netbanking={enable_netbanking}, mobile={mobile_number}")
                    if not all([name, password, deposit, mobile_number]):
                        ...
                        try:
                            ...
                            if deposit < 500:
                                response = {"status": "error", "message": "Minimum deposit ₹500"}
                            else:
                                cursor = conn.cursor()
                                cursor.execute("SELECT account_no FROM accounts WHERE mobile_number = ?", (mobile_number,))
                                existing_account = cursor.fetchone()
                                if existing_account:
                                    response = {"status": "error", "message": "Account exists with this mobile number", "account_exists": True, "existing_account_no": existing_account[0]}
                                else:
                                    ...
                                    cursor.execute("INSERT INTO accounts (account_no, name, password, balance, netbanking, puf_data, transactions, otp, mobile_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                                  (account_no, name, password, deposit, int(enable_netbanking), None, transactions, None, mobile_number))
                                    conn.commit()
                                    response = {"status": "success", "message": "Account registered", "account_no": account_no}
                                    self.log(f"Registered account {account_no} for {name}")
                        except Exception as e:
                    ...
                    password = data.get("password")
                    cursor = conn.cursor()
                    cursor.execute("SELECT password FROM accounts WHERE account_no = ?", (account_no,))
                    ...
                    account_no = data.get("account_no")
                    cursor = conn.cursor()
                    cursor.execute("SELECT mobile_number FROM accounts WHERE account_no = ?", (account_no,))
                    account_data = cursor.fetchone()
                    if account_data:
                        mobile_number = account_data[0]
                        if not mobile_number or len(mobile_number) != 10 or not mobile_number.isdigit():
                            response = {"status": "error", "message": "Invalid mobile number in database"}
                        else:
                            otp = str(abs(hash(f"{account_no}{datetime.now().strftime('%Y%m%d%H%M')}")) % 1000000).zfill(6)
                            if self.send_sms_via_fast2sms(otp, mobile_number):
                                cursor.execute("UPDATE accounts SET otp = ? WHERE account_no = ?", (otp, account_no))
                                conn.commit()
                                response = {"status": "success", "message": "OTP sent successfully."}
                                self.log(f"OTP {otp} sent to {mobile_number}")
                            else:
                                response = {"status": "error", "message": "Failed to send OTP. Check server logs."}
                    else:
                    ...
                    otp = data.get("otp")
                    cursor = conn.cursor()
                    cursor.execute("SELECT otp FROM accounts WHERE account_no = ?", (account_no,))
                    result = cursor.fetchone()
                    if result and result[0] == otp:
                        cursor.execute("UPDATE accounts SET otp = NULL WHERE account_no = ?", (account_no,))
                        conn.commit()
                        cursor.execute("SELECT account_no, name, balance, netbanking, puf_data, transactions, mobile_number FROM accounts WHERE account_no = ?", (account_no,))
                        result = cursor.fetchone()
                        if result:
                    ...
                    if account_no and puf_data:
                        cursor = conn.cursor()
                        cursor.execute("UPDATE accounts SET puf_data = ? WHERE account_no = ?", (puf_data, account_no))
                        conn.commit()
                        response = {"status": "success"}
                    ...
                    puf_data = data.get("puf_data")
                    cursor = conn.cursor()
                    cursor.execute("SELECT puf_data FROM accounts WHERE account_no = ?", (account_no,))
                    result = cursor.fetchone()
                    if result and result[0] == puf_data:
                        response = {"status": "success"}
                    else:
                        response = {"status": "error", "message": "PUF verification failed"}
    ...
    def start_server(self):
        ...
        try:
            ...
            self.stat_lbl.config(text=f"Status: Running on {self.host}:{self.port}", fg="#27ae60")
